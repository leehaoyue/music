...
<template>
  <mu-row class="collectlist">
    ...
    <mu-col span="12" class="collectlistinner">
      <mu-grid-list class="gridlist-demo" v-show="$store.state.collect.length>0">
        <mu-grid-tile v-for="(item, index) in $store.state.collect" :key="index"
            @click="playMic(item)">
      ...
      <noData :msg="'暂未收藏任何歌曲'" v-show="$store.state.collect.length<=0" />
    </mu-col>
    <reTop domName="collectlistinner"/>
  </mu-row>
</template>
